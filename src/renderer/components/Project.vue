            ...
            <Split v-else :class="{ 'empty': noRepositories || emptyStatus || frameworkLoading || repositoryMissing }">
                    ...
                    <section>
                        ...
                        >
                            <SidebarFramework
                                ...
                                :key="framework.id"
                                :class="{ 'is-active': model.active.framework === framework.id }"
                ...
                <Pane id="list">
                    ...
                    <template v-if="noRepositories">
                        <div class="cta">
                            <h2>{{ 'Add repositories to :0' | set(name) }}</h2>
...
export default {
    ...
    async created () {
        ...
        console.log(this.model, this.model.active.framework)
    ...
    methods: {
        save () {
            this.$root.saveProject(this.model)
        },
        ...
        onProjectChange () {
            ...
            if (!this.model.repositories.length) {
                ...
                this.model.repositories.forEach(repository => {
                    ...
                    repository.frameworks.forEach(framework => {
                        if (!setFramework || this.model.active.framework === framework.id) {
                ...
                if (setFramework) {
                    ...
                    this.onFrameworkChanged(this.framework.id)
                }
            }
        ...
        onFrameworkAdded (framework) {
            this.registerFrameworkListeners(framework)
            this.model.active.framework = framework.id
            this.onProjectChange()
        },
        onFrameworkChanged (frameworkId) {
            // Persist the active framework in the project.
            this.model.active.framework = frameworkId
            this.frameworkLoading = true
            this.$root.loadSuites(frameworkId).then(({ frameworkId, suites }) => {
                // Only inject suites and mark framework as mounted if the
                // active framework is still the one we set out to load.
                if (this.framework && this.model.active.framework === frameworkId) {
                    this.suites = suites
                    this.onFrameworkMounted()
                }
            })
            this.save()
        },
